    ...
    public void I_search_emailid_and_click_add_and_enter_the_below_details_and_click_save(DataTable convertToMember) throws Exception {
        ...
        for (Map<String, String> data : convertToMember.asMaps(String.class, String.class)) {
           ...
           // bniConnect.navigateMenu("Operations,Region");
            ...
            TimeUnit.SECONDS.sleep(2);
            viewEditVisitorsList.selectYear("2020");
            TimeUnit.SECONDS.sleep(2);
            ...
            TimeUnit.SECONDS.sleep(2);
            String applicationYear = data.get("applicationYear");
            String applicationMonth= data.get("applicationMonth");
            String applicationDay = data.get("applicationDay");
            String region = data.get("region");
            String chapter = data.get("chapter");
            String profession = data.get("profession");
            String requestedSpeciality = data.get("requestedSpeciality");
            String firstName = data.get("firstName");
            String addressLine1 = data.get("addressLine1");
            String country = data.get("country");
            String phone = data.get("phone");
            String paymentOption = data.get("paymentOption");
            String membershipOption = data.get("membershipOption");
            add.selectVisitYear(applicationYear);
            TimeUnit.SECONDS.sleep(2);
            add.selectVisitMonth(applicationMonth);
            TimeUnit.SECONDS.sleep(2);
            add.selectDateFromDatePicker(applicationDay);
            TimeUnit.SECONDS.sleep(2);
            add.selectRegion(region);
            TimeUnit.SECONDS.sleep(2);
            add.selectChapter(chapter);
            TimeUnit.SECONDS.sleep(2);
            add.selectProfession(profession);
            TimeUnit.SECONDS.sleep(2);
          //  add.selectSpeciality(data.get("speciality"));
            String speciality = data.get("speciality");
            System.out.println("Speciality" +speciality);
            add.selectSpeciality(speciality);
            {
                if (speciality.startsWith("z")) {
                    add.selectRequestedSpeciality(requestedSpeciality);
                }
                else{
                    System.out.println("Speciality does not start with z");            }
            }
            TimeUnit.SECONDS.sleep(2);
            ...
            TimeUnit.SECONDS.sleep(2);
            add.enterAddressLine1(addressLine1);
            TimeUnit.SECONDS.sleep(2);
            add.selectCountry(country);
            TimeUnit.SECONDS.sleep(2);
            add.enterPhone(phone);
            TimeUnit.SECONDS.sleep(2);
            add.selectPaymentOption(paymentOption);
            TimeUnit.SECONDS.sleep(2);
            add.selectMemberShipOption(membershipOption);
            TimeUnit.SECONDS.sleep(2);
            captureScreenShot = new CaptureScreenShot(driver);
