...
public class CreateNationalEvent {
    ...
    public void step_2(DataTable nationalEvent) throws Exception {
        ...
        for (Map<String, String> data : nationalEvent.asMaps(String.class, String.class)) {
            ...
            TimeUnit.SECONDS.sleep(1);
            createNewNationalEvent.enterCostForMember(data.get("costForMember"));
            TimeUnit.SECONDS.sleep(1);
            createNewNationalEvent.enterCostForNonMember(data.get("costForNonMember"));
            TimeUnit.SECONDS.sleep(1);
            ...
            TimeUnit.SECONDS.sleep(1);
            captureScreenShot = new CaptureScreenShot(driver);
            captureScreenShot.takeSnapShot(driver, "createNationalEvent");
           ...
           */
            ...
            String actualEventName = nationalManageEvents.checkEventName(data.get("eventName") + eventNameDateTime);
            Integer expEventName= dbConnect.queryRecordCount(readWritePropertyFile.loadAndReadPropertyFile("NationalEventName", "properties/sql.properties"));
            TimeUnit.SECONDS.sleep(2);
            assertEquals("Event name is not correct", expEventName, actualEventName);
            signOut.signOutBni();
            i++;
        }
    }
